...
with ds.keep_open("r") as read_ds:
    ...
    for group, j, conformer in tqdm(read_ds.chunked_items(max_size=2500), total=total_chunks, desc="Processing chunks"):
        ...
        # Compute per-atom standard deviation across ensemble predictions
        stdev_list = magnitudes.std(dim=0).squeeze().tolist()
        mean_rel_stdev = torch.tensor(stdev_list).mean().item()
        ...
        for i in range(n_structures):
            ...
            record = {
                ...
                "force_stdev": stdev_list,
                "mean_stdev": mean_rel_stdev,
            }
...
# Write to parquet
df.to_parquet("2xr_force-just-stdev.pq", index=False)

print("Done! Results stored in df and written to 2xr_force-just-stdev.pq")
